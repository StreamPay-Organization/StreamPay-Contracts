...
use crate::test_helpers::{self, bump_sequence, set_time, Setup, UninitializedSetup};
...
/// Test fixture structs and helpers live in `test_helpers`.
...
fn test_multiple_streams_have_independent_ids() {
    let (s, first) = Setup::new_with_stream();
    let second = s
        .contract
...
fn test_create_stream_before_initialize_fails() {
    ...
    let sender = u.new_address();
    let recipient = u.new_address();
    let res = u
        .contract
        .try_create_stream(&sender, &recipient, &1_000, &100, &200);
...
fn test_views_before_initialize_fail() {
    ...
    assert_eq!(u.contract.try_get_token(), Err(Ok(Error::NotInitialized)));
}
...
fn test_create_stream_at_exact_cap_succeeds() {
    let s = Setup::new_with_cap(1_000);
    let id = s
        .contract
        .create_stream(&s.sender, &s.recipient, &1_000, &100, &200);
    assert_eq!(id, 0);
...
fn test_top_up_blocked_by_supply_cap() {
    let s = Setup::new_with_cap(1_500);
    let id = s
        .contract
        .create_stream(&s.sender, &s.recipient, &1_000, &100, &200);
...
fn test_persistent_ttl_bump_on_top_up() {
    ...
    // Advance ledger so TTL naturally drops below the threshold, then top up to bump it again.
    bump_sequence(&s.env, 450_000);
...
fn test_upgrade_succeeds() {
    let s = Setup::new();
    // Register a valid fake Wasm code.
    let new_wasm_hash = s.env.deployer().upload_contract_wasm([0u8, 97, 115, 109, 1, 0, 0, 0]);
...
// --- Test helper self-verification ------------------------------------------

#[test]
fn test_helper_setup_creates_funded_initialized_contract() {
    let s = Setup::new();
    assert_eq!(s.contract.get_admin(), s.admin);
    assert_eq!(s.contract.get_token(), s.token.address);
    assert_eq!(s.contract.stream_counter(), 0);
...
#[test]
fn test_helper_uninitialized_setup_contract_rejects_create() {
    let u = UninitializedSetup::new();
    let sender = Address::generate(&u.env);
    let recipient = Address::generate(&u.env);
    assert_eq!(
        u.contract
            .try_create_stream(&sender, &recipient, &1_000, &100, &200),
...
#[test]
fn test_helper_uninitialized_setup_rejects_admin_query() {
    let u = UninitializedSetup::new();
    assert_eq!(u.contract.try_get_admin(), Err(Ok(Error::NotInitialized)));
}

#[test]
fn test_helper_setup_new_address_generates_unique_addresses() {
    let s = Setup::new();
    let a = s.new_address();
    let b = s.new_address();
    assert_ne!(a, b);
...
#[test]
fn test_helper_new_with_stream_deploys_stream() {
    let (s, id) = Setup::new_with_stream();
    assert_eq!(id, 0);
    assert_eq!(s.contract.stream_counter(), 1);
...
#[test]
fn test_helper_new_with_cap_sets_cap() {
    let cap = 5_000;
    let s = Setup::new_with_cap(cap);
    assert_eq!(s.contract.get_supply_cap(), cap);
}

// --- Event payload tests and helpers ----------------------------------------

fn val_eq(env: &Env, a: soroban_sdk::Val, b: soroban_sdk::Val) -> bool {
    let va: Vec<soroban_sdk::Val> = soroban_sdk::vec![env, a];
    let vb: Vec<soroban_sdk::Val> = soroban_sdk::vec![env, b];
    va == vb
}

fn contract_events(env: &Env, contract: &Address) -> std::vec::Vec<(Vec<soroban_sdk::Val>, soroban_sdk::Val)> {
    env.events()
        .all()
        .into_iter()
...
fn stream_topics(env: &Env, name: &str, id: u64) -> Vec<soroban_sdk::Val> {
    Vec::from_array(
        env,
...
fn admin_topics(env: &Env, name: &str) -> Vec<soroban_sdk::Val> {
...
fn test_admin_transfer_events_emit_payloads() {
    ...
    s.contract.schedule_admin_transfer(&s.admin, &first_admin);
    ...
    bump_sequence(&s.env, 172_801);

    s.contract.set_admin(&first_admin);
    let events = contract_events(&s.env, &s.contract.address);
